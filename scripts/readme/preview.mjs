...
const scene = ["reply", "revise", "ask"].includes(params.get("scene")) ? params.get("scene") : "reply";
...
if (params.get("mode") === "overlay") {
  ...
  Object.assign(contextCueApi, {
    onOverlayResult: (callback) => scene === "ask" ? () => {} : delayed(callback, {
      ...context, contact: example.person, candidates: candidates(example.candidates),
      scenario: "reply", detectedContact: example.person, detectedLanguage: locale,
      conversationSummary: example.messages.join(" "), memorySuggestions: [], generatedAt: "2026-09-03T09:41:00Z"
    }),
    onAskOpen: (callback) => scene === "ask" ? delayed(callback, context) : () => {},
    openAsk: async () => ({ ...context, canReturnToSuggestions: true }),
    onAskEvent: (callback) => { askListener = callback; return () => { askListener = undefined; }; },
    startAsk: (request) => setTimeout(() => askListener?.({ ...request, type: "complete", answer: example.answer }), 60),
  ...
  document.querySelector("#root").innerHTML = `
    <main class="stage stage--${scene}">
      ...
      <footer><span>${example.footer}</span><span>contextcue</span></footer>
    </main>`;
}
