<div align="center">
  ...
  <h1>ContextCue</h1>
  <p><strong>在任意窗口快速生成建议，开启 AI 对话。</strong></p>
...
只有用户主动唤起时，ContextCue 才会读取当前窗口的一张截图，结合少量相关本地记忆生成回复、文案或 AI 提示词。输入框不是前提：macOS 识别到可编辑控件时支持直接写入，没有识别到输入框时仍可生成并复制建议。
...
AI 问答默认使用 `⌘ ⇧ Enter` / `Ctrl ⇧ Enter`。浮窗显示前先保存当前页面截图，提交问题时不会重新截取已经切换的标签页。无法截图时会说明原因，并允许不带页面上下文继续提问。页面上下文可以关闭；关闭浮窗后截图和内存问答记录清除。
...
- 只有用户从当前窗口主动唤起 ContextCue 后才会截图。
